...
bool equivautA(vector<string> plateauEtalon, vector<string> plateauCompare, int largeur, int hauteur){
    if(plateauCompare.size() != plateauEtalon.size()){
        return false;
    }
    if(not est_anagramme(purifie(plateauEtalon[0]), purifie(plateauCompare[0]))){
        return false;
    }
    for(int i=1; i<plateauEtalon.size(); i++){
        if(plateauCompare[i].size() <= largeur and plateauEtalon[i].size() <= largeur){
